...
cp "$PROJECT_ROOT/target/release/vietcctl" "$STAGING/bin/"
cp "$PROJECT_ROOT/ui/target/release/vietc-tray" "$STAGING/bin/"
[ -f "$PROJECT_ROOT/target/release/vietc-xrecord" ] && cp "$PROJECT_ROOT/target/release/vietc-xrecord" "$STAGING/bin/"

# 4. Copy assets & support files
mkdir -p "$STAGING/systemd"
mkdir -p "$STAGING/ibus"
cp "$PROJECT_ROOT/packaging/99-vietc.rules" "$STAGING/udev/"
cp "$PROJECT_ROOT/packaging/deb/vietc.desktop" "$STAGING/desktop/"
cp "$PROJECT_ROOT/vietc.service" "$STAGING/systemd/"
cp "$PROJECT_ROOT/packaging/ibus/vietc.xml" "$STAGING/ibus/"
cp "$PROJECT_ROOT/vietc.toml" "$STAGING/config/config.toml"
cp "$PROJECT_ROOT/packaging/icons/"*.svg "$STAGING/icons/"
